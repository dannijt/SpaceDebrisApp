...
Finally I built a Shiny application with some visuals to explore the findings.  The timeseries plots & tables in the application come from the html widgets R package families reactable & dygraphs. These plots work just like regular R plots, but produce interactive web visualizations based on JavaScript libraries.   The dygraphs package for the timeseries plot also includes an option to add custom CSS which I included in the visualization. The code for the dygraphs and reactables visual are included in this repository. 

The 3D Render of the Globe illustrates space debris moving in real-time that was written in Javascript and embedded as an HTML document in the app. This was based off of the Three Globe Javascript Package. Description: A ThreeJS WebGL class to represent data visualization layers on a globe, using a spherical projection.
